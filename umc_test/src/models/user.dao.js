...
import { connectFoodCategory, confirmEmail, getUserID, insertUserSql, getPreferToUserID
    ...
    addUserPoint, getReviewByReviewId, getReviewByReviewIdAtFirst,
    getMissionByMissionIdAtFirst, getMissionByMissionId
 } from "./user.sql.js";
...
//특정 미션 완료하는 함수
export const completeUserMission = async (userId, missionId, size) => {
    try {
        console.log("오잉");
        const conn = await pool.getConnection();
        ...
        return getPreviewMission("undefined", size, userId);
    } catch (err) {
        throw new BaseError(status.PARAMETER_IS_WRONG);
    }
}
