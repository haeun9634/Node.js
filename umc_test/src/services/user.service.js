...
import { addUser, getUser, getUserPreferToUserID, setPrefer, addUserMission, getUserMissions, completeUserMission } from "../models/user.dao.js";
...
export const userjoinMission = async (body) => {
    const MissionUserData = await addUserMission({
        'user_id': body.user_id,
        'mission_id': body.mission_id,
    });


    if (MissionUserData == -1) { // 이미 존재할 경우 -1을 반환
        throw new BaseError(status.MISSION_ALREADY_EXIST);
    } else {
        if (body.complete == 1) {
            await completeUserMission(body.user_id, body.mission_id);
        }
        const userMissions = await getUserMissions(body.user_id);
        return userMissionResponseDTO(userMissions);
    }
};